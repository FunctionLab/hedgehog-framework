...
Welcome to the Wreath model repository! Wreath is a sequence-based deep learning model for prediction of methylation levels at CpG locus in the human genome across 296 methylation profiles available in the Berry dataset. This repository can be used to train, evaluate, and apply the Wreath model to make predictions on CpG locus disruptions to methylation levels given neighboring variants.  
...
(Wreath was originally trained with Python 3.8 with a local installation of Selene but we have since updated our requirements in the Selene package to require Python 3.9+.) 
...
- `train`: Trains the Wreath model architecture
- `eval`: Evaluates the Wreath model. Code can also be used to get Wreath
...
- `predict`: Variant effect prediction, where the change in methylation levels
             ...
             Wreath predictions given FASTA reference and alternative allele
             sequences as input.
